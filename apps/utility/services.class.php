...
class Services{
    ...
    public static function getBaseUrl(){
		...
		//unset($scriptName[sizeof($scriptName)-1]);
		//$scriptName = array_values($scriptName);
		//$vari = implode('/',$scriptName);
		$scriptName = explode('/',$_SERVER['SCRIPT_NAME']);
        $whitelist = array('127.0.0.1', '::1');
        if(self::ip_is_private($whitelist)==false){ $url = 'http://'.$_SERVER['SERVER_NAME'].'/'; } 
        if(self::ip_is_private($_SERVER['REMOTE_ADDR'])) { $url = 'http://'.$_SERVER['SERVER_NAME'].'/'.$scriptName[1].'/'; }
